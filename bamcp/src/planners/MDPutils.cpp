...
namespace MDPutils{
	...
	void policyEvaluation(uint S, 
		...
		do{
			...
			for(ll=0; ll<S; ++ll){
				if(!rsas)
					Vv[ll] = R[ll*A+PI[ll]] + gamma*utils::inner_prod(P+ll*SA+PI[ll]*S,Vu,S);
				else{
					Vv[ll] = 0;	
					uint aS = PI[ll]*S;
					uint lSA = ll*SA;
		...
		while(utils::sqnorm_2(V0,V1,S) > sqeps);
	...
	void valueIteration(uint S,
		...
		do{
			...
			for(ll=0;ll<S;++ll){
				...
				for(aa=0;aa<A;++aa){
					if(!rsas){
						Q[ll][aa] = R[ll*A+aa] + gamma*utils::inner_prod(P+lSA+aa*S,Vu,S);
					}
					else{
						Q[ll][aa] = 0;
						uint aS = aa*S;
