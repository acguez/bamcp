#include "experiment_sboss.h"
#include "boost/timer.hpp"

using namespace std;

EXPERIMENT_SBOSS::PARAMS::PARAMS()
:   NumRuns(100),
    NumSteps(300),
    SimSteps(1000),
    TimeOut(3600),
...
EXPERIMENT_SBOSS::EXPERIMENT_SBOSS(const SIMULATOR& real,
...
void EXPERIMENT_SBOSS::Run(std::vector<double>& Rhist)
{
    boost::timer timer;

    SBOSS boss(Simulator, SearchParams, samplerFact);
...
void EXPERIMENT_SBOSS::RunBandit(std::vector<uint>& Rhist, std::vector<uint>& optArm, uint bestArm)
{
    boost::timer timer;
